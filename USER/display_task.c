...
//    display_str_info->content[DISPLAY_ID_FP_ID_SUCCESS].str_len = sizeof("ÒÑ´ò¿ª");
//    memcpy(display_str_info->content[DISPLAY_ID_FP_ID_SUCCESS].str, "ÒÑ´ò¿ª", display_str_info->content[DISPLAY_ID_FP_ID_SUCCESS].str_len);
...
//    display_str_info->content[DISPLAY_ID_LOCK_STATUS].str_len = sizeof("lock status: ËøÒÑ¿ª£¬Çë³¢ÊÔ´ò¿ªÒ©ºÐ");
//    memcpy(display_str_info->content[DISPLAY_ID_LOCK_STATUS].str, "lock status: ËøÒÑ¿ª£¬Çë³¢ÊÔ´ò¿ªÒ©ºÐ", display_str_info->content[DISPLAY_ID_LOCK_STATUS].str_len);
...
//    display_str_info->content[DISPLAY_ID_TO_DESTINATION].str_len = sizeof("Ê¹ÓÃÕß:ÍõÒ½Éú, 10ºÅÊÖÊõÊÒ");
//    memcpy(display_str_info->content[DISPLAY_ID_TO_DESTINATION].str, "Ê¹ÓÃÕß:ÍõÒ½Éú, 10ºÅÊÖÊõÊÒ", display_str_info->content[DISPLAY_ID_TO_DESTINATION].str_len);
}
...
void show_fp_id_result(uint8_t result, uint16_t fp_id)
{
    ...
    if(result == TRUE)
    {
        display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str_len = sizeof("ÒÑ´ò¿ª");
        memcpy(display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str, "ÒÑ´ò¿ª", display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str_len);
    }
    else
    {
        display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str_color = Red;
        display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str_len = sizeof("Ö¸ÎÆ²»Æ¥Åä,½öÊÚÈ¨ÍõÒ½Éú¿ÉÒÔ´ò¿ª");
        memcpy(display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str, "Ö¸ÎÆ²»Æ¥Åä,½öÊÚÈ¨ÍõÒ½Éú¿ÉÒÔ´ò¿ª", display_str_info->content[DISPLAY_ID_FP_ID_STATUS].str_len);
    }
